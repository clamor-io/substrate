...
fn basic_minting_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1, true));
...
fn minting_too_many_insufficient_assets_fails() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, false, 1, true));
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 1, 1, false, 1, true));
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 2, 1, false, 1, true));
...
fn lifecycle_should_work() {
	new_test_ext().execute_with(|| {
		Balances::make_free_balance_be(&1, 100);
		assert_ok!(Assets::create(RuntimeOrigin::signed(1), 0, 1, 1, true));
		assert_eq!(Balances::reserved_balance(&1), 1);
...
fn min_balance_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 10, true));
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100));
...
fn destroy_accounts_calls_died_hooks() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 50, true));
...
fn force_asset_status_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::create(RuntimeOrigin::signed(1), 0, 1, 30, true));
...
fn balance_conversion_should_work() {
	new_test_ext().execute_with(|| {
		...
		let id = 42;
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), id, 1, true, 10, true));
		let not_sufficient = 23;
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), not_sufficient, 1, false, 10, true));
